...
pub fn remove(shortcut: String) -> Result<i32> {
    ...
    cfg.shortcuts.remove(index_to_remove);
    cfg.update()?;
...
pub fn remove_call(call: String) -> Result<i32> {
    ...
    if cfg.shortcuts[shortcut_index].calls.len() == 1 {
        return Err(anyhow!(format!(
            ...
            cfg.shortcuts[shortcut_index].name.yellow(),
        )));
    }

    cfg.shortcuts[shortcut_index].calls.remove(call_index);
    cfg.update()?;
